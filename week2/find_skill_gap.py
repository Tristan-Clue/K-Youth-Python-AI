...
if __name__ == "__main__":
	try:
		...
		common = list(set(gap.gaps).intersection(wrong))
		print(f"Common items with d3_wrong.json (Should be empty): {common}")
		with open("data/output.json", "w") as file:
			json.dump(gap.gaps, file, indent=2)
			print("Result outputted to data/output.json")
	except Exception as e:
		print(f"Error: {e}")
